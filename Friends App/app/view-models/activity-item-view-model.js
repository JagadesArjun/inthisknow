...
var ActivityItemViewModel = (function (_super){
    ...
    Object.defineProperty(ActivityItemViewModel.prototype, "tagImageSource", {
        get: function () {

            var that = this;
            var url = this._source.User.Type === null ? null : this._source.User.Type.Uri;

            if (!url) {
                this._tagImageSource = null;
            }
            else if (this._source && !this._tagImageSource) {
                var responsiveImagesUrl = getResponsiveUrl(url, 100);

                if (cache) {
                    var cachedImg = cache.get(responsiveImagesUrl);
                    if (cachedImg) {
                        that._tagImageSource = cachedImg;
                    }
                    else {
                        cache.push({
                            key: responsiveImagesUrl,
                            ...
                            completed: function (result, key) {
                                if (responsiveImagesUrl === key) {
                                    that._tagImageSource = result;
                                    that.notify({ object: that, eventName: observable.Observable.propertyChangeEvent, propertyName: "tagImageSource", value: that._tagImageSource });
                                }
                            }
                        });
                    }
    ...
    Object.defineProperty(ActivityItemViewModel.prototype, "defaultTagAvatarImageSource", {
        get: function () {
            var avatarImage = imageSource.fromFile("~/res/deftag.png");
            return avatarImage;
        },
        enumerable: true,
        configurable: true
